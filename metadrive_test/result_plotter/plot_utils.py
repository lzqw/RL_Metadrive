    ...
    if any(groups_results.keys()):
        if legend_group_num:
            legend = plt.legend(
                ...
                ['%s (%i)' % (key.replace('without', 'w/o').replace('_', '-'), groups_results[key]['num']) for key in
                ...
                ncol=len(groups_results.keys()) if legend_outside else 1)
        else:
            legend = plt.legend(
                [groups_results[key]['legend'] for key in groups_results.keys()],
...
def plot_data(data, xaxis='total_steps', value="mean_score", condition="Condition1", smooth=1,
    ...
    plt.legend(
        ...
        loc=9 if legend_outside else legend_loc,
        bbox_to_anchor=(0.5, -0.1) if legend_outside else (1, 1) if legend_outside else None,
        borderpad=legend_borderpad, labelspacing=legend_labelspacing, ncol=len(labels) - 1 if legend_outside else 1)
